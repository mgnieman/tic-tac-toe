...
var countXs = function countXs(array) {
  var xCount = 0;
...
var countOs = function countOs(array) {
  var oCount = 0;
  for (var i = 0; i < array.length; i++) {
    if (array[i] === 'O') {
      oCount++;
...
var checkForDraw = function checkForDraw() {
  var boxArray = logic.getBoxArray();
...
var chooseLetter = function chooseLetter(event) {
  var boxArray = logic.getBoxArray();
  var move = {
    game: {
      'cell': {
  ...
  if (logic.checkForWin() || $(event.target).html() === 'X' || $(event.target).html() === 'O') {
    ...
    if (countXs(boxArray) === countOs(boxArray)) {
      ...
      // console.log(event.target.id.charAt(3))
      var _draw = checkForDraw();
      var _win = logic.checkForWin();
      if (_draw || _win) {
        // logic.startNewGame()
        move.game.over = true;
      }
...
var startNewButton = function startNewButton(event) {
  event.preventDefault();
  api.newGame().then(ui.newGameSuccess).catch(ui.newGameFailure);
};
var onStoreNewMove = function onStoreNewMove(data) {
  api.storeNewMove(data).then(ui.storeMoveSuccess).catch(ui.storeMoveFailure);
};
...
var storeNewMove = function storeNewMove(data) {
  return $.ajax({
...
var storeMoveSuccess = function storeMoveSuccess(data) {
  console.log(data);
};
var storeMoveFailure = function storeMoveFailure() {
  console.log('Move not stored');
};
var getGamesSuccess = function getGamesSuccess(numGames) {
...
module.exports = {
  ...
  storeMoveSuccess: storeMoveSuccess,
  storeMoveFailure: storeMoveFailure,
...
var winMessage = function winMessage(xOrO) {
  if (xOrO === 'X') {
...
var getBoxArray = function getBoxArray() {
  return boxArray;
};
...
var startNewGame = function startNewGame() {
  ...
  var boxesChildren = $('.boxes').children().children();
  var counter = 0;
...
var checkForWin = function checkForWin() {
  if (same(boxArray[0], boxArray[1], boxArray[2]) || same(boxArray[3], boxArray[4], boxArray[5]) || same(boxArray[6], boxArray[7], boxArray[8]) || same(boxArray[0], boxArray[4], boxArray[8]) || same(boxArray[2], boxArray[4], boxArray[6]) || same(boxArray[0], boxArray[3], boxArray[6]) || same(boxArray[1], boxArray[4], boxArray[7]) || same(boxArray[2], boxArray[5], boxArray[8])) {
